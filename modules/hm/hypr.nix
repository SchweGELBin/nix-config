    ...
    wayland.windowManager.hyprland = {
      ...
      settings = {
        ...
        bindd = [
          ...
          "$mod1       ,  mouse:274 ,  Launch Auto Clicker ($clicker)                ,  exec                  , kitty --hold $clicker"
        ...
        ++ lib.optionals (cfg.land.layout == "dwindle") [
          "$mod1       ,  O         ,  Toggle h/w window split                       ,  layoutmsg             , togglesplit"
          "$mod1       ,  P         ,  Toggle pseudo (Keep aspect ratio)             ,  pseudo                , "
        ]
        ++ lib.optionals (cfg.land.layout == "master") [
        ...
        ++ lib.optionals (cfg.land.layout == "scrolling") [
          "$mod2       ,  left      ,  Decrease column size                          ,  layoutmsg             , colresize -0.1"
          "$mod2  SHIFT,  right     ,  Fit column to beginning                       ,  layoutmsg             , fit tobeg"
        ...
        monitorv2 = [
          ...
          {
            output = monitors.first.name;
            mode = "${toString vars.monitors.first.res.x}x${toString vars.monitors.first.res.y}@${toString vars.monitors.first.hz}";
            position = "0x0";
            scale = 1;
            bitdepth = lib.optional cfg.land.forceBitdepth.enable vars.monitors.first.bit;
          }
          {
            output = monitors.second.name;
            mode = "${toString vars.monitors.second.res.x}x${toString vars.monitors.second.res.y}@${toString vars.monitors.second.hz}";
            position = "${toString vars.monitors.first.res.x}x0";
            scale = 1;
